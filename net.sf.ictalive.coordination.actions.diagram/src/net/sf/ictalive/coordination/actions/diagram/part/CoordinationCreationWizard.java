...
public class CoordinationCreationWizard extends Wizard implements INewWizard {
	...
	public void init(IWorkbench workbench, IStructuredSelection selection) {
		...
		setDefaultPageImageDescriptor(ActionDiagramEditorPlugin
				.getBundledImageDescriptor("icons/wizban/NewActionsWizard.gif")); //$NON-NLS-1$
	...
	public void addPages() {
		diagramModelFilePage = new CoordinationCreationWizardPage(
				"DiagramModelFile", getSelection(), "actions_diagram"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		domainModelFilePage = new CoordinationCreationWizardPage(
				"DomainModelFile", getSelection(), "actions") { //$NON-NLS-1$ //$NON-NLS-2$

			public void setVisible(boolean visible) {
				if (visible) {
					String fileName = diagramModelFilePage.getFileName();
					fileName = fileName.substring(0, fileName.length()
							- ".actions_diagram".length()); //$NON-NLS-1$
					setFileName(CoordinationDiagramEditorUtil
							.getUniqueFileName(getContainerFullPath(),
									fileName, "actions")); //$NON-NLS-1$
				}
