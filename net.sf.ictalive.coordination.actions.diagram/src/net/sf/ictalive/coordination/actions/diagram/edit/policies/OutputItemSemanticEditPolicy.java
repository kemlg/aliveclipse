...
public class OutputItemSemanticEditPolicy extends
	...
	public OutputItemSemanticEditPolicy() {
		super(CoordinationElementTypes.Output_2027);
	}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (CoordinationVisualIDRegistry.getVisualID(incomingLink) == ProcessHasOutputEditPart.VISUAL_ID) {
			...
			if (CoordinationVisualIDRegistry.getVisualID(incomingLink) == BindingToParamEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(
						incomingLink.getSource().getElement(), null,
						incomingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
