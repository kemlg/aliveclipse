...
import control.diagram.edit.policies.IfThenElse6ItemSemanticEditPolicy;
 ...
 * @generated
 */
public class IfThenElse6EditPart extends ShapeNodeEditPart {
	 ...
	 * @generated
	 */
	public IfThenElse6EditPart(View view) {
		super(view);
	}
	...
	protected void createDefaultEditPolicies() {
		...
		installEditPolicy(EditPolicyRoles.SEMANTIC_ROLE,
				new IfThenElse6ItemSemanticEditPolicy());
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof IfThenElseName6EditPart) {
			((IfThenElseName6EditPart) childEditPart)
					.setLabel(getPrimaryShape()
							.getFigureIfThenElseLabelFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseIfConditionCompartment5EditPart) {
			IFigure pane = getPrimaryShape()
					.getIfThenElseIfConditionCompartmentFigure();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.add(((IfThenElseIfThenElseIfConditionCompartment5EditPart) childEditPart)
							.getFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseThenCompartment5EditPart) {
			IFigure pane = getPrimaryShape()
					.getIfThenElseThenCompartmentFigure();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.add(((IfThenElseIfThenElseThenCompartment5EditPart) childEditPart)
							.getFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseElseCompartment5EditPart) {
			IFigure pane = getPrimaryShape()
					.getIfThenElseElseCompartmentFigure();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.add(((IfThenElseIfThenElseElseCompartment5EditPart) childEditPart)
							.getFigure());
			return true;
		}
		return false;
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof IfThenElseIfThenElseIfConditionCompartment5EditPart) {
			...
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.remove(((IfThenElseIfThenElseIfConditionCompartment5EditPart) childEditPart)
							.getFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseThenCompartment5EditPart) {
			...
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.remove(((IfThenElseIfThenElseThenCompartment5EditPart) childEditPart)
							.getFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseElseCompartment5EditPart) {
			...
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.remove(((IfThenElseIfThenElseElseCompartment5EditPart) childEditPart)
							.getFigure());
			return true;
		}
		return false;
