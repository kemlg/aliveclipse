...
import control.diagram.edit.policies.IfThenElseItemSemanticEditPolicy;
...
public class IfThenElseEditPart extends ShapeNodeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof IfThenElseNameEditPart) {
			((IfThenElseNameEditPart) childEditPart).setLabel(getPrimaryShape()
					.getFigureIfThenElseLabelFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseIfConditionCompartment7EditPart) {
			IFigure pane = getPrimaryShape()
					.getIfThenElseIfConditionCompartmentFigure();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.add(((IfThenElseIfThenElseIfConditionCompartment7EditPart) childEditPart)
							.getFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseThenCompartment7EditPart) {
			IFigure pane = getPrimaryShape()
					.getIfThenElseThenCompartmentFigure();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.add(((IfThenElseIfThenElseThenCompartment7EditPart) childEditPart)
							.getFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseElseCompartment7EditPart) {
			IFigure pane = getPrimaryShape()
					.getIfThenElseElseCompartmentFigure();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.add(((IfThenElseIfThenElseElseCompartment7EditPart) childEditPart)
							.getFigure());
			return true;
		}
		return false;
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof IfThenElseIfThenElseIfConditionCompartment7EditPart) {
			...
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.remove(((IfThenElseIfThenElseIfConditionCompartment7EditPart) childEditPart)
							.getFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseThenCompartment7EditPart) {
			...
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.remove(((IfThenElseIfThenElseThenCompartment7EditPart) childEditPart)
							.getFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseElseCompartment7EditPart) {
			...
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.remove(((IfThenElseIfThenElseElseCompartment7EditPart) childEditPart)
							.getFigure());
			return true;
		}
		return false;
