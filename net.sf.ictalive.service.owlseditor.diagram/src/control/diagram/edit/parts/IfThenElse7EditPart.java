...
import control.diagram.edit.policies.IfThenElse7ItemSemanticEditPolicy;
 ...
 * @generated
 */
public class IfThenElse7EditPart extends ShapeNodeEditPart {
	 ...
	 * @generated
	 */
	public IfThenElse7EditPart(View view) {
		super(view);
	}
	...
	protected void createDefaultEditPolicies() {
		...
		installEditPolicy(EditPolicyRoles.SEMANTIC_ROLE,
				new IfThenElse7ItemSemanticEditPolicy());
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof IfThenElseName7EditPart) {
			((IfThenElseName7EditPart) childEditPart)
					.setLabel(getPrimaryShape()
							.getFigureIfThenElseLabelFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseIfConditionCompartment6EditPart) {
			IFigure pane = getPrimaryShape()
					.getIfThenElseIfConditionCompartmentFigure();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.add(((IfThenElseIfThenElseIfConditionCompartment6EditPart) childEditPart)
							.getFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseThenCompartment6EditPart) {
			IFigure pane = getPrimaryShape()
					.getIfThenElseThenCompartmentFigure();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.add(((IfThenElseIfThenElseThenCompartment6EditPart) childEditPart)
							.getFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseElseCompartment6EditPart) {
			IFigure pane = getPrimaryShape()
					.getIfThenElseElseCompartmentFigure();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.add(((IfThenElseIfThenElseElseCompartment6EditPart) childEditPart)
							.getFigure());
			return true;
		}
		return false;
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof IfThenElseIfThenElseIfConditionCompartment6EditPart) {
			...
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.remove(((IfThenElseIfThenElseIfConditionCompartment6EditPart) childEditPart)
							.getFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseThenCompartment6EditPart) {
			...
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.remove(((IfThenElseIfThenElseThenCompartment6EditPart) childEditPart)
							.getFigure());
			return true;
		}
		if (childEditPart instanceof IfThenElseIfThenElseElseCompartment6EditPart) {
			...
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane
					.remove(((IfThenElseIfThenElseElseCompartment6EditPart) childEditPart)
							.getFigure());
			return true;
		}
		return false;
