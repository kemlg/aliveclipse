/**
 * <copyright>
 * </copyright>
 *
 * $Id$
 */
package net.sf.ictalive.opera.OM;

import org.eclipse.emf.ecore.EObject;

/**
 * <!-- begin-user-doc -->
 * A representation of the model object '<em><b>OM</b></em>'.
 * <!-- end-user-doc -->
 *
 * <p>
 * The following features are supported:
 * <ul>
 *   <li>{@link net.sf.ictalive.opera.OM.OM#getSs <em>Ss</em>}</li>
 *   <li>{@link net.sf.ictalive.opera.OM.OM#getIs <em>Is</em>}</li>
 *   <li>{@link net.sf.ictalive.opera.OM.OM#getNs <em>Ns</em>}</li>
 *   <li>{@link net.sf.ictalive.opera.OM.OM#getCs <em>Cs</em>}</li>
 * </ul>
 * </p>
 *
 * @see net.sf.ictalive.opera.OM.OMPackage#getOM()
 * @model
 * @generated
 */
public interface OM extends EObject
{
	/**
	 * Returns the value of the '<em><b>Ss</b></em>' containment reference.
	 * <!-- begin-user-doc -->
	 * <p>
	 * If the meaning of the '<em>Ss</em>' containment reference isn't clear,
	 * there really should be more of a description here...
	 * </p>
	 * <!-- end-user-doc -->
	 * @return the value of the '<em>Ss</em>' containment reference.
	 * @see #setSs(SS)
	 * @see net.sf.ictalive.opera.OM.OMPackage#getOM_Ss()
	 * @model containment="true" required="true"
	 * @generated
	 */
	SS getSs();

	/**
	 * Sets the value of the '{@link net.sf.ictalive.opera.OM.OM#getSs <em>Ss</em>}' containment reference.
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 * @param value the new value of the '<em>Ss</em>' containment reference.
	 * @see #getSs()
	 * @generated
	 */
	void setSs(SS value);

	/**
	 * Returns the value of the '<em><b>Is</b></em>' containment reference.
	 * <!-- begin-user-doc -->
	 * <p>
	 * If the meaning of the '<em>Is</em>' containment reference isn't clear,
	 * there really should be more of a description here...
	 * </p>
	 * <!-- end-user-doc -->
	 * @return the value of the '<em>Is</em>' containment reference.
	 * @see #setIs(IS)
	 * @see net.sf.ictalive.opera.OM.OMPackage#getOM_Is()
	 * @model containment="true" required="true"
	 * @generated
	 */
	IS getIs();

	/**
	 * Sets the value of the '{@link net.sf.ictalive.opera.OM.OM#getIs <em>Is</em>}' containment reference.
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 * @param value the new value of the '<em>Is</em>' containment reference.
	 * @see #getIs()
	 * @generated
	 */
	void setIs(IS value);

	/**
	 * Returns the value of the '<em><b>Ns</b></em>' containment reference.
	 * <!-- begin-user-doc -->
	 * <p>
	 * If the meaning of the '<em>Ns</em>' containment reference isn't clear,
	 * there really should be more of a description here...
	 * </p>
	 * <!-- end-user-doc -->
	 * @return the value of the '<em>Ns</em>' containment reference.
	 * @see #setNs(NS)
	 * @see net.sf.ictalive.opera.OM.OMPackage#getOM_Ns()
	 * @model containment="true" required="true"
	 * @generated
	 */
	NS getNs();

	/**
	 * Sets the value of the '{@link net.sf.ictalive.opera.OM.OM#getNs <em>Ns</em>}' containment reference.
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 * @param value the new value of the '<em>Ns</em>' containment reference.
	 * @see #getNs()
	 * @generated
	 */
	void setNs(NS value);

	/**
	 * Returns the value of the '<em><b>Cs</b></em>' containment reference.
	 * <!-- begin-user-doc -->
	 * <p>
	 * If the meaning of the '<em>Cs</em>' containment reference isn't clear,
	 * there really should be more of a description here...
	 * </p>
	 * <!-- end-user-doc -->
	 * @return the value of the '<em>Cs</em>' containment reference.
	 * @see #setCs(CS)
	 * @see net.sf.ictalive.opera.OM.OMPackage#getOM_Cs()
	 * @model containment="true" required="true"
	 * @generated
	 */
	CS getCs();

	/**
	 * Sets the value of the '{@link net.sf.ictalive.opera.OM.OM#getCs <em>Cs</em>}' containment reference.
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 * @param value the new value of the '<em>Cs</em>' containment reference.
	 * @see #getCs()
	 * @generated
	 */
	void setCs(CS value);

} // OM
