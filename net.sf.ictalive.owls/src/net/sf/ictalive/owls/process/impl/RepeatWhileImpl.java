...
public class RepeatWhileImpl extends IterateImpl implements RepeatWhile {
	...
	public NotificationChain basicSetWhileProcess(ControlConstruct newWhileProcess, NotificationChain msgs) {
		...
		if (eNotificationRequired()) {
			ENotificationImpl notification = new ENotificationImpl(this, Notification.SET, ProcessPackage.REPEAT_WHILE__WHILE_PROCESS, oldWhileProcess, newWhileProcess);
			if (msgs == null) msgs = notification; else msgs.add(notification);
		}
		return msgs;
	}
	...
	public void setWhileProcess(ControlConstruct newWhileProcess) {
		if (newWhileProcess != whileProcess) {
			...
			if (whileProcess != null)
				msgs = ((InternalEObject)whileProcess).eInverseRemove(this, EOPPOSITE_FEATURE_BASE - ProcessPackage.REPEAT_WHILE__WHILE_PROCESS, null, msgs);
			if (newWhileProcess != null)
				msgs = ((InternalEObject)newWhileProcess).eInverseAdd(this, EOPPOSITE_FEATURE_BASE - ProcessPackage.REPEAT_WHILE__WHILE_PROCESS, null, msgs);
			msgs = basicSetWhileProcess(newWhileProcess, msgs);
			if (msgs != null) msgs.dispatch();
		}
		else if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, ProcessPackage.REPEAT_WHILE__WHILE_PROCESS, newWhileProcess, newWhileProcess));
	}
