...
import net.sf.ictalive.coordination.agents.diagram.part.CoordinationDiagramUpdater;
import net.sf.ictalive.coordination.agents.diagram.part.CoordinationLinkDescriptor;
import net.sf.ictalive.coordination.agents.diagram.part.CoordinationNodeDescriptor;
 ...
 * @generated
 */
public class MasModelCanonicalEditPolicy extends CanonicalConnectionEditPolicy {
	...
	protected List getSemanticChildrenList() {
		...
		for (Iterator it = CoordinationDiagramUpdater
				.getMasModel_1000SemanticChildren(viewObject).iterator(); it
				.hasNext();) {
	...
	protected Set getFeaturesToSynchronize() {
		if (myFeaturesToSynchronize == null) {
			...
			myFeaturesToSynchronize.add(AgentsPackage.eINSTANCE
					.getMasModel_Agents());
		}
	...
	private Collection collectAllLinks(View view, Map domain2NotationMap) {
		if (!MasModelEditPart.MODEL_ID.equals(CoordinationVisualIDRegistry
				.getModelID(view))) {
			return Collections.EMPTY_LIST;
		}
		...
		case MasModelEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CoordinationDiagramUpdater
						.getMasModel_1000ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
		...
		case AgentEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CoordinationDiagramUpdater
						.getAgent_2004ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case AtomicActionEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CoordinationDiagramUpdater
						.getAtomicAction_2006ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case CompositeActionEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(CoordinationDiagramUpdater
						.getCompositeAction_2007ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
